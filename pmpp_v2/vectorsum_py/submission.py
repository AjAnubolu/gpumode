...
# CUB DeviceReduce::Sum wrapper. Same pattern as sort:
# - Raw uintptr_t for zero-overhead Python->C++ dispatch
# - Workspace cache keyed by (dtype, n)
import torch
...
template <typename T>
void _sum(uintptr_t in_ptr, uintptr_t out_ptr, int n,
          uintptr_t ws, size_t ws_bytes) {
    cub::DeviceReduce::Sum(
        reinterpret_cast<void*>(ws), ws_bytes,
        reinterpret_cast<const T*>(in_ptr),
        reinterpret_cast<T*>(out_ptr),
        n);
}
...
void sum_f32(uintptr_t,uintptr_t,int64_t,uintptr_t,size_t);
void sum_i32(uintptr_t,uintptr_t,int64_t,uintptr_t,size_t);
void sum_i64(uintptr_t,uintptr_t,int64_t,uintptr_t,size_t);
"""

_mod = load_inline(name="cub_reduce_sum", cpp_sources=_CPP_SRC, cuda_sources=_CUDA_SRC,
                   functions=["ws_f32","ws_i32","ws_i64","sum_f32","sum_i32","sum_i64"],
...
def custom_kernel(data: input_t) -> output_t:
    ...
    getattr(_mod, f"sum_{key}")(x.data_ptr(), out.data_ptr(), n, ws.data_ptr(), ws.numel())
    return out
